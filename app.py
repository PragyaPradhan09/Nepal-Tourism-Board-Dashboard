    ...
    with tab4:
        ...
        if not run_button:
            ...
            st.markdown('<div class="warn-box">⚠️ Post-COVID Indian arrivals '
                        '(2023–2025: 319K–438K) exceed pre-COVID levels (254K max in 2019). '
                        'A chronological (last-N-years) split puts the entire surge in the '
                        'test set, which the model never saw in training — this collapses '
                        'test R². To let the model train on at least some surge-range values, '
                        'this split uses a <b>random shuffle</b> (train_test_split) instead of '
                        'a strict time-ordered split. This means the result below is no longer '
                        'a genuine forecast evaluation (it tests interpolation, not prediction '
                        'of unseen future years) — treat it as a structural/coefficient check, '
                        'and use SARIMA for actual Indian forecasting.</div>',
                        unsafe_allow_html=True)
            ...
            with st.spinner("Fitting Ridge Regression for Indian..."):
                ...
                # Try multiple random splits to check how stable/unstable the
                # result is on this small dataset, rather than relying on one
                # arbitrary seed that may luck into an easy or hard test set.
                seeds = [0, 1, 7, 21, 42]
                split_results = []
                for seed in seeds:
                    X_tr, X_te, y_tr, y_te, yr_tr, yr_te = train_test_split(
                        X_ind, y_ind, yrs_ind,
                        test_size=n_test_years, random_state=seed, shuffle=True)
                    sc_tmp = StandardScaler()
                    X_tr_sc = sc_tmp.fit_transform(X_tr)
                    X_te_sc = sc_tmp.transform(X_te)
                    model_tmp = RidgeCV(alphas=[0.01,0.1,1,5,10,50,100,500,1000], cv=5)
                    model_tmp.fit(X_tr_sc, y_tr)
                    test_r2 = r2_score(y_te, model_tmp.predict(X_te_sc))
                    split_results.append({
                        "seed": seed,
                        "test_years": sorted(yr_te.tolist()),
                        "test_r2": test_r2
                    })

                # Use the seed closest to the median test R² as the "representative" split.
                # We deliberately pick the median (not the best-looking) result so the
                # headline numbers below aren't cherry-picked.
                split_results_sorted = sorted(split_results, key=lambda d: d["test_r2"])
                median_split = split_results_sorted[len(split_results_sorted)//2]
                chosen_seed = median_split["seed"]

                X_ind_train, X_ind_test, y_ind_train, y_ind_test, yrs_ind_train, yrs_ind_test = \
                    train_test_split(X_ind, y_ind, yrs_ind,
                                      test_size=n_test_years, random_state=chosen_seed, shuffle=True)

                sci   = StandardScaler()
                Xsci_train = sci.fit_transform(X_ind_train)
                Xsci_test  = sci.transform(X_ind_test)
                ridgei = RidgeCV(alphas=[0.01,0.1,1,5,10,50,100,500,1000], cv=5)
                ridgei.fit(Xsci_train, y_ind_train)

                yptri = ridgei.predict(Xsci_train)
                yptei = ridgei.predict(Xsci_test)

            st.caption(f"Random split (shuffled, not chronological) — "
                       f"Train years: {sorted(yrs_ind_train.tolist())} | "
                       f"Test years: {sorted(yrs_ind_test.tolist())} "
                       f"(seed={chosen_seed}, chosen as the median-performing split across "
                       f"{len(seeds)} seeds tried)")

            with st.expander("📊 Stability check across random seeds"):
                seed_df = pd.DataFrame(split_results)
                st.dataframe(seed_df.style.format({"test_r2": "{:.4f}"}),
                             use_container_width=True)
                r2_min = seed_df["test_r2"].min()
                r2_max = seed_df["test_r2"].max()
                st.caption(f"Test R² range: {r2_min:.3f} to {r2_max:.3f} — "
                           f"a wide range here means the result is highly sensitive to "
                           f"which {n_test_years} year(s) happen to fall in the test set, "
                           f"not a reliable model property.")
                st.markdown(
                    f"**Interpretation:** With only {len(sub_ind)} annual observations and "
                    f"several distinct regimes in the data (pre-growth years, the COVID crash, "
                    f"and the post-COVID surge), any small random test split is effectively a "
                    f"coin flip on whether the held-out years resemble the training data. "
                    f"The {r2_max - r2_min:.1f}-point swing in test R² across just {len(seeds)} "
                    f"seeds (from {r2_min:.2f} to {r2_max:.2f}) shows that Ridge Regression's "
                    f"apparent performance on Indian arrivals is **not stable** — it depends on "
                    f"which years happen to be excluded, not on a consistent underlying fit. "
                    f"This instability is itself a meaningful finding: it supports using SARIMA, "
                    f"which models temporal structure directly, as the primary forecasting tool "
                    f"for Indian arrivals, while treating this Ridge model as a structural / "
                    f"coefficient-interpretation exercise rather than a forecasting model."
                )

            mi_train = get_metrics(y_ind_train, yptri)
            mi_test  = get_metrics(y_ind_test, yptei)
